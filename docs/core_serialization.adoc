...
An archive has a header with two fields: a magic constant of 16 bits (the two characters `'g'` and `'f'`) and a version integer of 16 bits encoded in big endian. Then data is serialized as specified by the serialization operators. Basic types are handled directly by the serializer and deserializer through direct methods. Complex types are handled by <<_operator_pipe>>.
