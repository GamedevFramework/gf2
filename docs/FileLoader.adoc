...
`FileLoader` provides a generic loader, so you should use xref:core_resources.adoc#_loader_for[`loader_for`] in order to use it with xref:ResourceRegistry.adoc[`ResourceRegistry`].
...
Generic loader for a resource in a file. The resource type `T` should provide a constructor with a `const std::filesystem::path&` argument.
