...
// MSVC does not like extern template
  ...
  constexpr
  auto vec(Tx x, Ty y) -> Vec2<std::common_type_t<Tx, Ty>> {
  ...
  constexpr
  Vec2<std::common_type_t<T, U>> operator+(Vec2<T> lhs, Vec2<U> rhs) {
  ...
  template<typename T, typename U>
  constexpr
  Vec2<std::common_type_t<T, U>> operator+(Vec2<T> lhs, U rhs) {
    return Vec2<std::common_type_t<T, U>>(lhs.x + rhs, lhs.y + rhs);
  }

  template<typename T, typename U>
  constexpr
  Vec2<T>& operator+=(Vec2<T>& lhs, U rhs) {
    lhs.x += rhs;
    lhs.y += rhs;
    return lhs;
  }
  ...
  template<typename T, typename U>
  constexpr
  Vec2<std::common_type_t<T, U>> operator+(T lhs, Vec2<U> rhs) {
  ...
  template<typename T>
  constexpr
  T dot(Vec2<T> lhs, Vec2<T> rhs) {
    return lhs.x * rhs.x + lhs.y * rhs.y;
  }

  template<typename T>
  constexpr
  T manhattan_length(Vec2<T> vec) {
    return details::abs(vec.x) + details::abs(vec.y);
  }

  template<typename T>
  constexpr
  T manhattan_distance(Vec2<T> lhs, Vec2<T> rhs) {
    return manhattan_length(lhs - rhs);
  }

  template<typename T>
  constexpr
  T square_length(Vec2<T> vec) {
    return square(vec.x) + square(vec.y);
  }

  template<typename T>
  constexpr
  T square_distance(Vec2<T> lhs, Vec2<T> rhs) {
    return square_length(lhs - rhs);
  }

  template<typename T>
  inline
  auto euclidean_length(Vec2<T> vec) {
    return std::hypot(vec.x, vec.y);
  }

  template<typename T>
  inline
  auto euclidean_distance(Vec2<T> lhs, Vec2<T> rhs) {
  ...
  template<typename T>
  constexpr
  T chebyshev_distance(Vec2<T> lhs, Vec2<T> rhs) {
    return chebyshev_length(lhs - rhs);
  }

  template<typename T>
  constexpr
  T natural_length(Vec2<T> vec) {
    return manhattan_length(vec) + square_length(vec);
  }

  template<typename T>
  constexpr
  T natural_distance(Vec2<T> lhs, Vec2<T> rhs) {
  ...
  template<typename T>
  constexpr
  Vec2<T> projx(Vec2<T> vec) {
    return { vec.x, T(0) };
  }

  template<typename T>
  constexpr
  Vec2<T> projy(Vec2<T> vec) {
    return { T(0), vec.y };
  }

  template<typename T>
  constexpr
  Vec2<T> dirx(T length) {
    return { length, T(0) };
  }

  template<typename T>
  constexpr
  Vec2<T> diry(T length) {
    return { T(0), length };
  }
  ...
  template<typename T>
  constexpr
  Vec2<T> perp(Vec2<T> vec) {
    return { -vec.y, vec.x };
  }

  template<typename T>
  constexpr
  Vec2<T> triple_product(Vec2<T> a, Vec2<T> b, Vec2<T> c) {
    return dot(a, c) * b - dot(a, b) * c;
  }

  template<typename T>
  constexpr
  Vec2<T> inverse_triple_product(Vec2<T> a, Vec2<T> b, Vec2<T> c) {
    return - dot(c, b) * a + dot(c, a) * b;
  }

  template<typename T>
  constexpr
  T cross(Vec2<T> lhs, Vec2<T> rhs) {
    return lhs.x * rhs.y - lhs.y * rhs.x;
  }

  template<typename T>
  inline
  T angle_to(Vec2<T> lhs, Vec2<T> rhs) {
    return std::atan2(cross(lhs, rhs), dot(lhs, rhs));
  }

  template<typename T>
  constexpr
  Vec2<T> project_onto(Vec2<T> vec, Vec2<T> axis) {
    return dot(vec, axis) / square_length(axis) * axis;
  }

  template<typename Archive, typename T>
  inline
  Archive& operator|(Archive& ar, Vec2<T>& vec) {
