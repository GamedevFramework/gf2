...
inline namespace v1 {
  ...
  public:
    ...
    template<typename U, typename = std::enable_if_t<std::is_convertible_v<U(*)[], T(*)[]>>>
    constexpr
    StaticSpan(std::array<U, N>& values) noexcept
    : m_data(values.data())
    {
    }
    ...
    constexpr
    Span<T> first_except(std::size_t count) const noexcept {
      assert(count <= Size);
    ...
    template<std::size_t C>
    constexpr
    StaticSpan<T, Size - C> first_except() const noexcept {
      static_assert(C <= Size, "Out of bounds.");
      return slice<0, Size - C>();
    }

    constexpr
    Span<T> last_except(std::size_t count) const noexcept {
      return slice(count, Size);
    }

    template<std::size_t C>
    constexpr
    StaticSpan<T, Size - C> last_except() const noexcept {
      static_assert(C <= Size, "Out of bounds.");
      return slice<C, Size>();
    }
  ...
  public:
    ...
    template<std::size_t C>
    constexpr
    Span<T> first_except() const noexcept {
      assert(C <= m_size);
    ...
    template<std::size_t C>
    constexpr
    Span<T> last_except() const noexcept {
      assert(C <= m_size);
