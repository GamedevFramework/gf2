...
namespace gf {
  ...
  void Camera::update(Vec2I framebuffer_size) {
    ...
    switch (type) {
      ...
      case CameraType::Fit: {
        ...
        Vec2F viewport_size = framebuffer_size * expected_viewport.extent;
        const float framebuffer_ratio = viewport_size.w / viewport_size.h;
  ...
  RectI Camera::compute_viewport(Vec2I framebuffer_size) const {
    RectI viewport;
    viewport.offset.x = static_cast<int>(framebuffer_size.w * computed_viewport.offset.x + 0.5f);
    viewport.offset.y = static_cast<int>(framebuffer_size.h * computed_viewport.offset.y + 0.5f);
    viewport.extent.w = static_cast<int>(framebuffer_size.w * computed_viewport.extent.w + 0.5f);
    viewport.extent.h = static_cast<int>(framebuffer_size.h * computed_viewport.extent.h + 0.5f);
    return viewport;
  }
