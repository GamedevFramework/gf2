...
namespace gf {
  ...
  NinePatchEntity::NinePatchEntity(GpuTexture* texture, Vec2F size, NinePatchLimits limits, RenderManager* render_manager)
  : m_nine_patch(texture, size, limits, render_manager)
  {
  }

  NinePatchEntity::NinePatchEntity(GpuTexture* texture, const NinePatchData& data, RenderManager* render_manager)
